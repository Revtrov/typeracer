// these are the quotes you can add some if you want
// they must be inside double quotes, "", and after each quote after the ""
// you must put a comma and put a space at the end of your text e.g. "hello ",
let randomQuote;


//readies some variable for use later
let arrayLength;
let ranNum;
let textOut;
let out;
...
fetch("https://type.fit/api/quotes")
    ...
    .then((data) => {
        ...
        const inputCorrectQeury = setInterval(() => {
            ...
            preloadImages([`https://source.unsplash.com/random/400x400?sig=${Math.floor(Math.random()*500)}`])

        }, 100);
        setInterval(() => {
            preloadImages([`https://source.unsplash.com/random/400x400?sig=${Math.floor(Math.random()*100000)}`])
        })
        setInterval(() => {
            preloadImages([`https://source.unsplash.com/random/400x400?sig=${Math.floor(Math.random()*100000)}`])
        })
